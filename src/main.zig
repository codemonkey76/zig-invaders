...
pub fn main() void {
    ...
    while (!rl.windowShouldClose()) {
        ...
        if (rl.isKeyPressed(rl.KeyboardKey.space)) {
            for (&bullets) |*bullet| {
                if (!bullet.active) {
                    bullet.position_x = player.position_x + player.width / 2;
                    ...
                    if (invader.alive and rl.getRandomValue(0, 100) < enemyShootChance) {
                        for (&enemy_bullets) |*bullet| {
                            if (!bullet.active) {
                                ...
                                bullet.position_y = invader.position_y - bullet.height;
                                bullet.active = true;
                                break;
                            }
                        }
            ...
            for (&invaders) |*row| {
                for (row) |*invader| {
                    if (invader.alive) {
                        ...
                        if (next_x < 0 or next_x + invader.width > @as(f32, @floatFromInt(rl.getScreenWidth()))) {
                            hit_edge = true;
                            break;
                        }
                    }
